<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Pied Piper - Articles</title>
    <link rel="stylesheet" href="style.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@100&display=swap" rel="stylesheet">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Foldit&display=swap" rel="stylesheet">
</head>
<body>
    <div class="container">
        <div class="navbar">
            <img src="../img/logo.png" class="logo">
            <nav>
                <ul>

                    <li><a href="../index.html">Home</a></li>
                    <li><a href="../quiz/quiz.html">Quiz</a></li>
                    <li><a href="articles.html">Articles</a></li>

                </ul>
            </nav>
            <div class="dropdown">
                <img src="../img/menu.png" class="menu-icon" alt="drop-down menu">
                <div class="dropdown-content">
                    <a href="../login/login.html">Log In</a>
                    <a href="../form/form.html">Sign up</a>      
                    
                   
                
                </div>
            </div>
      </div>

    <div class="artists-section">
    <h2>Featured Artists</h2>

        <div class="artist">
        <a href="../albumsnartists/Mitski/albums.html"><img src="../img/image-1.jpeg" alt="Artist 1"></a>
        <p>Mitski</p>
        </div>

        <div class="artist">
        <a href="../albumsnartists/Yung Lean/albums.html"><img src="../img/image-2.jpeg" alt="Artist 2"></a>
        <p>Yung Lean</p>
        </div>

        <div class="artist">
           <a href="../albumsnartists/The Cranberries/albums.html"><img src="../albumsnartists/img/cranb.jpg" alt="Artist 4"></a> 
            <p>The Cranberries</p>
            </div>

        <div class="artist">
        <a href="../albumsnartists/Kanye West/albums.html"><img src="../img/image-4.jpg" alt="Artist 3"></a>
        <p>Kanye West</p>
        </div>

        
    </div>
    <br>

        <p class="checkout">Check out these articles!</p>
        <div class="row">
            <div class="artikulli">
                <a href="../albumsnartists/Starz/albums.html">
                    <img src="../img/starz.png" alt="f1">
                    <p>Starz - Yung Lean</p>
                </a>
            </div>

            <div class="artikulli">
                <a href="../albumsnartists/808s and Heartbreak/albums.html">
                    <img src="../img/808s_&_Heartbreak.png" alt="f2">
                    <p>Kanye West - 808s & Heartbreak</p>
                </a>
            </div>

            <div class="artikulli">
                <a href="../albumsnartists/Man On The Moon 2/albums.html">
                    <img src="../img/manonthemoon2.jpg" alt="f3">
                    <p>Kid Cudi - Man on the Moon II: The Legend of Mr. Rager</p>
                </a>
            </div>

            <div class="artikulli">
                <a href="../albumsnartists/Selected Ambient Works/albums.html">
                    <img src="../img/Selected_Ambient_Works_85-92.png" alt="f3">
                    <p>Aphex Twin - Selected Ambient Works 85–92</p>
                </a>
            </div>

            <div class="artikulli">
                <a href="../albumsnartists/Pastel Blues/albums.html">
                    <img src="../img/Ninasimonepastelblues.jpg" alt="f3">
                    <p>Nina Simone - Pastel Blues</p>
                </a>
            </div>

            <div class="artikulli">
                <a href="../albumsnartists/333/albums.html">
                    <img src="../img/Bladee_333_album_art.jpeg" alt="f3">
                    <p>Bladee - 333</p>
                </a>
            </div>
        </div>

    <script src="script.js"></script>
</body>
</html>
